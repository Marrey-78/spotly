...
import { ArrowLeft, Calendar, Clock, Euro, Plus, Ticket, MapPin, Trash2 } from 'lucide-react';
import { Button } from './ui/button';
import { Input } from './ui/input';
import { getOrganizerEvents, createOrganizerEvent, deleteEvent} from '../api/events';
...
interface OrganizerEventsManagerViewProps {
  organizer: Organizer;
  onBack: () => void;
}
...
export function OrganizerEventsManagerView({
  organizer,
  onBack,
}: OrganizerEventsManagerViewProps) {
...
const loadEvents = async () => {
      try {
        ...
        console.error(error);
        alert('Errore caricamento eventi organizzatore');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      alert('Errore creazione evento organizzatore');
    }
  };
      ...
      {events.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-md p-6 text-center">
          ...
          <p className="text-gray-600 text-sm">
            Crea il primo evento per questa organizzazione.
          </p>
        </div>
      ) : (
